...
function updateDashboardStreams() {
	$.ajax('/plugins/jdownloaderstatus/ajax.php')
		.done(function (streams) {
			$('#jdownloaderstatus_count').html(streams.length);
			...
			if (streams.length > 0) {
					...
					if ($container.length === 0) {
						$container = $(
							'<tr style="display:table-row;" id="' +
								stream.id +
								'">' +
								'<td width="40%" style="padding: 0px;"><p class="plexstream-title" title="' +
								stream.title +
								'">' +
								stream.title +
								'</p></td>' +
								'<td align="center" style="padding: 0px;text-align:center;"><i class="fa fa-' +
								stream.stateIcon +
								'" title="' +
								stream.state +
								'"></i></td>' +
								'<td align="center" style="padding: 0px;"><p class="plexstream-user" title="' +
								stream.user +
								'">' +
								stream.user +
								'</td>' +
								'<td align="center" style="padding: 0px;text-align:right;"><p class="plexstream-time"><span class="currentPositionHours">' +
								stream.currentPositionHours.toString().padStart(2, 0) +
								'</span>:<span class="currentPositionMinutes">' +
								stream.currentPositionMinutes.toString().padStart(2, 0) +
								'</span>:<span class="currentPositionSeconds">' +
								stream.currentPositionSeconds.toString().padStart(2, 0) +
								'</span> / ' +
								stream.lengthDisplay +
								'</div></td>' +
								'</tr>'
						).appendTo('#jdownloaderstatus_streams');
						...
						$($cells[1])
							.find('i')
							.attr('class', 'fa fa-' + stream.stateIcon)
				...
				$('#jdownloaderstatus_streams tr[updatedat]').each(function () {
				...
				$('#jdownloaderstatus_streams').html(
					'<tr class="no_streams"><td colspan="4" align="center" style="padding: 0 0 0 0;"><p style="text-align:center;font-style:italic;">' +
						_('There are currently no active streams') +
						'</p></td></tr>'
				);
			}
		})
		.fail(function (jqXHR) {
			if (jqXHR.status == '500') {
				$('#jdownloaderstatus_streams').html(
					'<tr><td colspan="4" align="center"><p style="text-align:center;font-style:italic;">' +
						_('Please make sure you have') +
						' <a href="/Settings/JDownloaderStatus">' +
						_('setup') +
						'</a> ' +
						_('the plugin first') +
						'</p></td></tr>'
...
function updateFullStreamInfo() {
	$.ajax('/plugins/jdownloaderstatus/ajax.php')
			...
			if (streams.length > 0) {
				...
				streams.forEach(function (stream) {
					...
					if ($container.length > 0) {
						...
						$container = $(
							'<li class="stream-container" id="' +
								stream.id +
								'"><div class="stream-subcontainer"><div class="stream" style="background-image:url(' +
								stream.artUrl +
								');"><div class="blur"><div class="details"><ul class="detail-list"><li><div class="label">' +
								_('Length') +
								'</div><div class="value">' +
								stream.duration +
								'</div></li><li><div class="label">' +
								_('Stream') +
								'</div><div class="stream value">' +
								stream.streamDecision +
								'</div></li><li><div class="label">' +
								_('Location') +
								'</div><div class="value" title="' +
								stream.locationDisplay +
								'" style="pointer:default;">' +
								stream.locationDisplay +
								'</div></li><li><div class="label">' +
								_('Bandwidth') +
								'</div><div class="bandwidth value">' +
								stream.bandwidth +
								'</div></li><li><div class="label">' +
								_('Audio') +
								'</div><div class="audio value">' +
								stream.streamInfo.audio['@attributes'].decision +
								'</div></li><li>' +
								(stream.streamInfo.video
									? '<div class="label">' + _('Video') + '</div><div class="video value">' + stream.streamInfo.video['@attributes'].decision + '</div></li>'
									: '') +
								'</ul></div><div class="poster" style="background-image:url(' +
								stream.thumbUrl +
								');"></div><div class="userIcon" title="' +
								stream.user +
								'" style="background-image:url(' +
								stream.userAvatar +
								')"></div></div></div><div class="bottom-box"><div class="progressBar" duration="' +
								stream.duration +
								'" style="' +
								stream.percentPlayed +
								'%;"><div class="position"><span class="currentPositionHours">' +
								stream.currentPositionHours.toString().padStart(2, 0) +
								'</span>:<span class="currentPositionMinutes">' +
								stream.currentPositionMinutes.toString().padStart(2, 0) +
								'</span>:<span class="currentPositionSeconds">' +
								...
								'</span>  / ' +
								stream.lengthDisplay +
								'</div></div><div class="title"><a href="#" onclick="openBox(\'/plugins/jdownloaderstatus/movieDetails.php?details=' +
								encodeURIComponent(stream.key) +
								'&host=' +
								encodeURIComponent(stream['@host']) +
								"','Details',600,900); return false;\">" +
								stream.title +
								'</a><div class="status"><i class="fa fa-' +
								stream.stateIcon +
								'" title="' +
								stream.status +
		...
		.fail(function (jqXHR) {
			if (jqXHR.status == '500') {
				$('#jdownloaderstatus_streams').html(
					'<tr><td colspan="4" align="center"><p style="text-align:center;font-style:italicl">' +
						_('Please make sure you have') +
						' <a href="/Settings/JDownloaderStatus">' +
						_('setup') +
						'</a> ' +
...
function getServers(containerSelector, selected) {
	var url = '/plugins/jdownloaderstatus/getServers.php?useSsl=' + $('input[name="FORCE_PLEX_HTTPS"]:checked').val();
			...
			for (var id in serverList) {
				if (serverList.hasOwnProperty(id)) {
					...
					serverList[id].Connections.forEach(function (connection) {
						if (connection !== null) {
							$host.append(
								'<input type="checkbox" onchange="updateServerList(\'HOST\')" name="hostbox" id="' +
									connection.uri +
									'" data-id="' +
									id +
									'"' +
									(selected.indexOf(connection.uri) > -1 ? ' checked="checked"' : '') +
									' value="' +
									connection.uri +
									'"/> <label for="' +
									connection.uri +
...
function getPlexHeaders() {
	return {
		...
		'X-Plex-Product': 'Unraid JDownloader Status Plugin',
		'X-Plex-Version': PLUGIN_VERSION,
		'X-Plex-Client-Identifier': getLocalStorage('UnraidJDownloaderStatus_ClientID', uuidv4(), false),
		'X-Plex-Platform': 'unraid',
		'X-Plex-Platform-Version': OS_VERSION,
		'X-Plex-Model': 'Plex OAuth',
		'X-Plex-Device': OS_VERSION,
		'X-Plex-Device-Name': 'Unraid JDownloader Status Plugin',
